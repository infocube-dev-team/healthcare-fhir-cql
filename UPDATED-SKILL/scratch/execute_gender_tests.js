...
const testPatients = [
  { id: 'patient-male', file: 'patient-male.json' },
  { id: 'patient-female', file: 'patient-female.json' }
];

async function runTest(patient) {
  const patientFile = path.join(__dirname, patient.file);
  const cqlContent = fs.readFileSync(path.join(__dirname, '..', 'GenderEligibility.cql'), 'utf8');
