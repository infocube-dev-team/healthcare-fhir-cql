...
const testPatients = [
  { id: 'patient-tablet', file: 'patient-tablet.json' },
  { id: 'patient-smartphone', file: 'patient-smartphone.json' }
];

async function runTest(patient) {
  const patientFile = path.join(__dirname, patient.file);
  const cqlContent = fs.readFileSync(path.join(__dirname, '..', 'TabletUserEligibility.cql'), 'utf8');
