...
const testPatients = [
  { id: 'patient-postgrad', file: 'patient-postgrad.json' },
  { id: 'patient-undergrad', file: 'patient-undergrad.json' }
];

async function runTest(patient) {
  const patientFile = path.join(__dirname, patient.file);
  const cqlContent = fs.readFileSync(path.join(__dirname, '..', 'EducationEligibility.cql'), 'utf8');
