...
To build and POST, first define the `get_cql_endpoint` inline function in the current shell session (generated by the skill — no file written to disk), then call it to obtain the URL:

```bash
# 1. Define the inline function (ephemeral — current session only, no .env access by the AI)
get_cql_endpoint() {
...
> The endpoint URL is obtained through an inline `get_cql_endpoint` shell function generated by the skill at the start of each session. The function reads `CQL_SANDBOX_URL` from the environment and prints only the URL — no file is written to disk and the AI never reads `.env` files. Never hard-code the URL in skill content or committed files.
